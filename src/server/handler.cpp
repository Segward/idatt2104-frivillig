...
// See include/server/handler.hpp for the public API.
//
// The wire uses "element_id" rather than "id" because the auth envelope
// already owns "id" (the assigned replica ID). Each message has a "type"
// discriminator; decoders return nullopt on mismatch, which is what makes
// process_message's try-each-envelope dispatch work without exception-driven
// control flow. Decoders validate every field before constructing output, so
// a malformed payload yields a clean nullopt rather than partial state.
...
namespace {
  ...
  // Parses `json_text`, verifies it's an object with `type` == `expected_type`,
  // and returns it. Returns nullopt on any mismatch — the server's
  // try-each-envelope dispatch depends on this never throwing.
  std::optional<json> parse_envelope(const std::string& json_text,
                                     std::string_view expected_type) {
    auto parsed = json::parse(json_text, nullptr, false);
    if (parsed.is_discarded() || !parsed.is_object()) return std::nullopt;
    auto type_it = parsed.find("type");
    if (type_it == parsed.end() || !type_it->is_string()) return std::nullopt;
    if (type_it->get<std::string>() != expected_type) return std::nullopt;
    return parsed;
  }
  ...
  // Skips zero entries when shipping a PN-counter state, so a freshly-seeded
  // replica produces an empty bucket that peers accept as a no-op merge.
  nlohmann::json non_zero_counts(const std::unordered_map<std::string, std::uint64_t>& bucket) {
    auto obj = nlohmann::json::object();
    for (const auto& [id, amount] : bucket) {
      if (amount != 0) obj[id] = amount;
    }
    return obj;
  }
}
...
std::string Handler::encode_counter(const CounterPNState& state) {
  nlohmann::json msg = {
    {"type", "counter_state"},
    {"increments", non_zero_counts(state.increments)},
    {"decrements", non_zero_counts(state.decrements)},
  };
  return msg.dump();
}

std::optional<CounterPNState> Handler::decode_counter(const std::string& json_text) {
  auto parsed = parse_envelope(json_text, "counter_state");
...
namespace {
  ...
  // Empty element_id is reserved for the list head sentinel and rejected.
...
std::optional<ListRGAState> Handler::decode_list_state(const std::string& json_text) {
  auto parsed = parse_envelope(json_text, "list_state");
  if (!parsed) return std::nullopt;
...
std::optional<TextRGAState> Handler::decode_text_state(const std::string& json_text) {
  auto parsed = parse_envelope(json_text, "text_state");
  if (!parsed) return std::nullopt;
  auto nodes_it = parsed->find("nodes");
  if (nodes_it == parsed->end() || !nodes_it->is_array()) return std::nullopt;
  TextRGAState out;
