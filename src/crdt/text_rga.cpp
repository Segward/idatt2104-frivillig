...
bool TextRGA::node_exists(const std::string& node_id) const {
    return _nodes.contains(node_id);
}
...
// already-present character is treated as a successful no-op (idempotent
...
bool TextRGA::try_apply_change(const TextChange& change) {
    if (change.type == TextOperationType::insert_op) {
        ...
        _nodes.emplace(change.element_id, TextCharacter{
...
// Multi-pass fixed-point loop: a character can only be inserted once its
